...
if go = 1 {
	var length = point_direction(gui_x, gui_y, 2400, 800);
}else{
	var length = point_direction(gui_x, gui_y, 2400+(changex*10), 800);
}	

x+=lengthdir_x(spd, length);
y+=lengthdir_y(spd, length);

if point_distance(2400, 800, gui_x, gui_y) < spd+100 {
	instance_destroy(self);	
	global.fishes++;
}

draw_sprite(spr_fish, 0, gui_x, gui_y);